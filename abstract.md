...
## Results
The proposed metric produces smooth and interpretable synchrony signals over time for both local player groups and larger team units. Across different group definitions, GSI highlights moments of high and low coordination. High synchronization values typically appear when players adjust their movement direction and timing together in relation to the ball, while low values indicate more independent or uncoordinated behavior. Visualizations and animations help explain these patterns by linking the numeric score to player positions on the pitch.
